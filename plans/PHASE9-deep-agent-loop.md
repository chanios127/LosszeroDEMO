...
| 9.4 | 시스템 프롬프트 가이드 + AgentLoop 도구 등록 + SSE subagent_* 이벤트 **+ Debug Fix 3·4 흡수** | BackEnd Infra |
| 9.5 | 프론트 인라인 ReportContainer 렌더링 + persistence(메시지 메타데이터) **+ Debug Fix 2 흡수** | Front/View + BackEnd Infra |
...
### Debug 사이클 흡수 항목 (2026-04-29 추가)

직전 Debug 사이클(AS현안 4턴 정합도 평가)에서 본질 원인으로 짚힌 Fix 2·3·4를 위 sub-phase에 통합. Fix 1·5는 Debug가 자율 C로 별도 처리(Phase 9와 무충돌).

**9.4에 흡수**:
- **Fix 3** — `backend/agent/loop.py:199-202` 도구 결과 wrapping에 `[meta] rows={N}` prepend. LLM 자체 truncation의 가시성 확보.
- **Fix 4** — `backend/prompts/system_base.md`에 에러 회복 가이드 한 줄 추가: "쿼리가 'Invalid column name' 에러를 반환하면 컬럼명을 추측하지 말고 도메인 스키마(시스템 메시지) 또는 `list_tables`를 다시 참조".

**9.5에 흡수**:
- **Fix 2** — `backend/main.py:500` `_conversations[session_id]` 저장 형식 풍부화. AgentLoop가 사용한 messages 리스트(시스템 제외)를 복사해 다음 턴이 `assistant(tool_use)` + `tool(result)` 페어를 그대로 보도록. 본 phase의 Persistence 설계(ReportSchema+ViewBundle 메시지 메타데이터 첨부)와 **동일 저장소** 재설계 영역이므로 두 변경을 함께 처리.
  - **위험**: `MAX_CONVERSATION_HISTORY=20`(메시지 수) 트리밍이 `tool_use`/`tool_result` 페어를 깨면 OpenAI/Anthropic 400 에러 발생. 페어 단위로 자르는 가드 필수.
  - 9.5는 결과적으로 Front/View(ReportContainer 인라인 렌더링) + BackEnd Infra(저장 형식 풍부화) 양 영역 작업 → 위임 시 두 agent 분담 또는 backend 우선 → frontend 단계 분할.

**회귀 점검에 추가**: 9.4·9.5 통합 후 다음 시나리오 재현 검증
- AS현안 4턴: Q1 거래처별 현황 조회 → Q2 옥스 분류 → Q3 최근 3개월 재고/자재/오류 키워드 + 긴급도 → Q4 Top 3 치명적 이슈
- 검증 포인트: Q3 SQL이 `wb_Title`/`wb_reqDt`/`wb_emFg` 실제 컬럼명 사용, 모든 턴에서 system_total_len > 0 유지, 도구 결과 행수와 LLM 자칭 행수 일치.

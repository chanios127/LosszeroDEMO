...
- `gantt` — time markers per row.
  - **span mode** (start + end 둘 다 있음, 예: 출퇴근 / 작업 일정): emit `chart` with `x: "<label_col>"`, `y: ["<start_col>", "<end_col>"]` (배열 — 두 컬럼 이름).
  - **anchor mode** (단일 시각만, 예: 출근 시각만 / 이벤트 발생 시점): emit `chart` with `x: "<label_col>"`, `y: "<time_col>"` (단일 string). 컴포넌트가 자동으로 anchor 마커로 렌더 (15분 작은 segment).
  - 절대로 `group_by`에 시간 컬럼을 넣지 말 것 — `group_by`는 색상 그룹 전용 (팀/상태/카테고리 등).
  - 시각 컬럼은 `"08:30"` / `"08:30:00"` / `"083000"` 등 HH:MM 또는 HHMMSS 형식 모두 허용 — 컴포넌트가 정규식 매치.
...
2. `chart` viz_hint=`"gantt"` — 직원별 출근 타임라인.
   - 출근/퇴근 둘 다 있을 때 (span): `{"viz_hint":"gantt","x":"이름","y":["출근시각","퇴근시각"],"group_by":"팀"}`.
   - 출근만 사용할 때 (anchor): `{"viz_hint":"gantt","x":"이름","y":"출근시각","group_by":"팀"}` — 컴포넌트가 15분 anchor marker로 자동 렌더.

...
- Always followed immediately by `build_view` in the same chain. Do NOT call `build_view` without first running `build_report`, and never construct ReportSchema by hand inside an `build_view` call.
...
| `build_report failed after 2 attempts` | Surface the underlying validation error to the user; do NOT try to construct `build_view` input by hand. Fall back to a plain markdown answer using the raw query results. |
